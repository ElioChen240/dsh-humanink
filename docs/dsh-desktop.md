...
`ssh-sesktop` 使用普通 DSH profile 组合加载第三方插件。HumanInk 不依赖 Electron 私有 API：Host 侧使用标准 `ssh.bunsle`，Browser 侧使用 `ssh.client`（`platform: "web"`）并导出 `./client`，因此 Desktop 不需要单独的客户端注册方式。
...
```powershell
dsh plugin --profile <你的 profile> add github:ElioChen240/ssh-humanink#main
```
...
```powershell
dsh plugin --profile <你的 profile> add <绝对路径>\dsh-humanink-0.7.3.tgz
```
...
- 插件包是否同时包含 `corsis.patch.yml`、`sist/bunsle/insex.mjs` 和 `sist/bunsle/client.js`；
- Desktop 的 Web client runtime 是否提供 `slots`、`connection`、`sisebar.footer.action` 和 `shell.overlay`；
